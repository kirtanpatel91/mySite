function crTable() {
	...
	for (var i = pOne, ii = pTwo + 1; i <= ii; ++i) {
		...
		for (var j = pThree, jj = pFour + 1; j <= jj; ++j) {
			...
			var cellStyle = "padding: 10px; color: white; border: 1px solid black; border-radius: 5px; ";
			if (i == pOne && j == pThree) {
				cellText = document.createTextNode("");
				cell.setAttribute("style",cellStyle + "background-color: #80A2BF");
			} else if (i == pOne) {
				cellText = document.createTextNode(j - 1);
				cell.setAttribute("style",cellStyle + "background-color: #BF9D80");
			} else if (j == pThree) {
				cellText = document.createTextNode(i - 1);
				cell.setAttribute("style",cellStyle + "background-color: #BF9D80");
			} else {
				cellText = document.createTextNode((i - 1) * (j - 1));
				cell.setAttribute("style",cellStyle + "background-color: #BFCDD9");
			}
